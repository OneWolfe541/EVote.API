...
namespace EVote.API.Extensions
{
    internal static class VoterDataExtensions
    {
        ...
        internal static IQueryable<T> BarCodeEquals<T>(this IQueryable<T> queryable, int? BarCode) where T : VoterDataModel
        {
            if (BarCode == null || queryable == null) return queryable;
            else
        ...
        internal static IQueryable<T> MaidenNameStartsWith<T>(this IQueryable<T> queryable, string strMaidenName) where T : VoterDataModel
        {
            if (strMaidenName.IsNullOrEmpty() || queryable == null) return queryable;
            else
                return queryable.Where(arg =>
                    arg.MaidenName.StartsWith(strMaidenName)
                    );
        }
        ...
        {
            if (strDate.IsNullOrEmpty() || queryable == null || strDate == "//") return queryable;
            else
            {
        ...
        internal static IQueryable<T> BirthYearContains<T>(this IQueryable<T> queryable, string strYear) where T : VoterDataModel
        {
            if (strYear.IsNullOrEmpty() || queryable == null) return queryable;
                ...
                return queryable.Where(arg => arg.DOBSearch.Contains(strYear));
        }

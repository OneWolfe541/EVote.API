...
namespace EVote.API.Controllers
{
    ...
    public class BallotStyleController : Controller
    {
        [HttpGet]
        public async Task<IActionResult> Get(string SQLDatabaseName)
        {
            try
            {
                ...
                {
                    var rtn = await context.BallotStyles.AsNoTracking().ToListAsync();
                    if (rtn == null)
            ...
            catch (Exception e)
            {
                return Ok(new ServisApiResponseViewModel
                {
                    ...
                    Message = "Critical failure while moving Ballot Styles",
                    Success = false
                });
            }
        }

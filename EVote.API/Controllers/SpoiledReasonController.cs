...
namespace EVote.API.Controllers
{
    ...
    public class SpoiledReasonController : Controller
    {
        [HttpGet]
        public async Task<IActionResult> Get(string SQLDatabaseName)
        {
            try
            {
                ...
                {
                    var rtn = await context.SpoiledReasons.AsNoTracking().ToListAsync();
                    if (rtn == null)
            ...
            catch (Exception e)
            {
                return Ok(new ServisApiResponseViewModel
                {
                    ...
                    Message = "Critical failure while moving Spoiled Reasons",
                    Success = false
                });
            }
        }

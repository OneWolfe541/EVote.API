...
namespace EVote.API.Controllers
{
    ...
    public class BallotStyleJurisdictionController : Controller
    {
        [HttpGet]
        public async Task<IActionResult> Get(string SQLDatabaseName)
        {
            try
            {
                ...
                {
                    var rtn = await context.BallotStyleJurisdictions.AsNoTracking().ToListAsync();
                    if (rtn == null)
            ...
            catch (Exception e)
            {
                return Ok(new ServisApiResponseViewModel
                {
                    ...
                    Message = "Critical failure while moving Ballot Style Jurisdictions",
                    Success = false
                });
            }
        }
